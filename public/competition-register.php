...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($is_from_uol === 'yes') {
        ...
        if (empty($other_university)) {
            $errors[] = "Please enter your university name.";
        } else {
            $university = $other_university;
        }
    } else {
        $errors[] = "Please specify if you are from University of Lahore.";
    }
        ...
        try {
            ...
            if ($checkEmail->fetch()) {
                ...
                $stmt = $db->prepare("
                    ...
                    (competition_id, team_name, team_leader_name, team_leader_email, team_leader_phone, team_leader_sapid, team_leader_department, university, team_members) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                ");
                    ...
                    <form method="post" id="registrationForm">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="is_from_uol" id="uol_yes" value="yes" required <?php echo (isset($_POST['is_from_uol']) && $_POST['is_from_uol'] === 'yes') ? 'checked' : ''; ?>>
                                <label class="form-check-label" for="uol_yes">Yes</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="is_from_uol" id="uol_no" value="no" required <?php echo (isset($_POST['is_from_uol']) && $_POST['is_from_uol'] === 'no') ? 'checked' : ''; ?>>
                                <label class="form-check-label" for="uol_no">No</label>
                            </div>
                        </div>

                        <div class="mb-3" id="other_university_div" style="display: none;">
                            ...
                            <input type="text" class="form-control" id="other_university" name="other_university" 
                                   value="<?php echo isset($_POST['other_university']) ? htmlspecialchars($_POST['other_university']) : ''; ?>"
...
document.addEventListener('DOMContentLoaded', function() {
    // Show/hide other university field based on radio selection
    ...
    addMemberField(); // add one default member field
    <?php endif; ?>
});
