...
class PDFGenerator {
    ...
    /**
     * Generate ID Card PDF for a member – now accepts dynamic QR image data
     */
    public function generateIDCardPDF($user, $qrImageData = null) {
        try {
            $options = new Options();
            $options->set('isHtml5ParserEnabled', true);
            $options->set('isRemoteEnabled', true);
            ...
            $html = $this->getIDCardHTML($user, $qrImageData);
            
            $dompdf->loadHtml($html, 'UTF-8');
            $dompdf->setPaper(array(0, 0, 340, 480), 'portrait');
            $dompdf->render();
    ...
    /**
     * Modified ID Card HTML – now accepts dynamic QR image data
     */
    private function getIDCardHTML($user, $qrImageData = null) {
        // Get profile picture
        $profile_pic = '';
        ...
        // QR code – dynamic if provided, otherwise fallback to static
        $qr_code = '';
        if ($qrImageData !== null && !empty($qrImageData)) {
            $qr_code = 'data:image/png;base64,' . base64_encode($qrImageData);
        } else {
            // Fallback to static QR image
        ...
        // User posts
        $posts_html = '';
        if (isset($user['posts']) && is_array($user['posts']) && !empty($user['posts'])) {
            $posts_str = strtoupper(implode(', ', $user['posts']));
            $posts_html = "
                        <div class=\"info-row\">
                            <div class=\"label\">POSITION</div>
                            <div class=\"position-badge\">$posts_str</div>
                        </div>";
        }
            ...
            <div class=\"id-card-container\">
                <div class=\"id-card-header\">
                    <div class=\"logo-container\">
                        " . ($mes_logo ? "<img src=\"$mes_logo\" class=\"mes-logo\" alt=\"MES Logo\">" : "<div style=\"width: 35px; height: 35px; background: #FF6600; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: #000; font-weight: bold; font-size: 12px; margin-right: 10px;\">MES</div>") . "
                ...
                <div class=\"id-card-body\">
                    <div class=\"id-photo-section\">
                        <div class=\"photo-container\">
                            " . ($profile_pic ? "<img src=\"$profile_pic\" class=\"photo\" alt=\"Photo\">" : "<div class=\"photo-placeholder\">$initials</div>") . "
                        </div>
                        <div class=\"validity\">Valid: " . date('Y') . " - " . (date('Y') + 1) . "</div>
                    </div>
                    <div class=\"id-info-section\">
                        <div class=\"info-row\"><div class=\"label\">FULL NAME</div><div class=\"value\">" . strtoupper($user['name']) . "</div></div>
                        <div class=\"info-row\"><div class=\"label\">SAP ID</div><div class=\"sap-value\">{$user['sap_id']}</div></div>
                        <div class=\"info-row\"><div class=\"label\">DEPARTMENT</div><div class=\"dept-value\">" . strtoupper($user['department']) . "</div></div>
                        <div class=\"info-row\"><div class=\"label\">SEMESTER</div><div class=\"sem-value\">{$user['semester']}</div></div>
                        $posts_html
                    </div>
                    <div class=\"id-qr-section\">
                        <div class=\"qr-container\">
                            " . ($qr_code ? "<img src=\"$qr_code\" class=\"qr-code\" alt=\"QR Code\">" : "<div class=\"qr-placeholder\">QR CODE<br>SCAN TO VERIFY</div>") . "
                        </div>
    ...
    private function getWelcomeLetterHTML($userData) {
        // This method is copied exactly from your original file to ensure no regression.
        $siteUrl = "http://" . $_SERVER['HTTP_HOST'] . dirname(dirname($_SERVER['PHP_SELF']));
        $currentDate = date('F j, Y');
        ...
        $clubLogoPath = realpath(__DIR__ . '/../../assets/images/Clubs-and-Societies-UOL-Logo.jpg');
        ...
        $clubLogo = $this->imageToBase64($clubLogoPath);
        ...
        // Ensure password is set and displayed
        ...
        <body>
            ...
            <div class=\"user-info\">
                <table class=\"info-table\">
                     ...
                     <tr><td>SAP ID:</td><td class=\"highlight\">" . ($userData['sap_id'] ?: 'Not Provided') . "</td></tr>
                     <tr><td>Department:</td><td class=\"highlight\">" . ($userData['department'] ?: 'Not Provided') . "</td></tr>
                     <tr><td>Semester:</td><td class=\"highlight\">" . ($userData['semester'] ?: 'Not Provided') . "</td></tr>
                     <tr><td>Email:</td><td class=\"highlight\">{$userData['email']}</td></tr>
                     <tr><td>Phone:</td><td class=\"highlight\">" . ($userData['phone'] ?: 'Not Provided') . "</td></tr>
            ...
            <div class=\"credentials\">
                ...
                <table style=\"width: 100%; font-size: 13px;\">
                     <tr>
                        ...
                        <td>{$siteUrl}/public/login.php</td>
                    </tr>
                    <tr>
                ...
                <div class=\"signature-section\">
                    ...
                    <div style=\"margin-top: 15px;\">
                        ...
                        <div class=\"signature-text\">
                            <strong>MES Society <br>
                            Department of Mechanical Engineering<br>
                            University of Lahore <br>
                            Email: mesuolofficial@gmail.com | Phone: +92 313 3150346 </strong>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class=\"footer\">
                     ...
                     <tr>
                        <td class=\"footer-left\">
                            <div class=\"footer-logo-item\">
                                " . ($mesLogo ? "<img src=\"$mesLogo\" class=\"footer-logo\" alt=\"MES Society\">" : "") . "
                                <span class=\"footer-logo-text\">MES Society &nbsp;</span>
                            </div>
                        </td>
                        <td class=\"footer-center\">
                            <div class=\"footer-logo-item\">
                                " . ($clubLogo ? "<img src=\"$clubLogo\" class=\"footer-logo\" alt=\"Office of Club & Societies\">" : "") . "
                                <span class=\"footer-logo-text\">Office of Club & Societies &nbsp;</span>
                            </div>
                        </td>
                        <td class=\"footer-right\">
                            <div class=\"footer-logo-item\">
                                " . ($universityLogo ? "<img src=\"$universityLogo\" class=\"footer-logo\" alt=\"University of Lahore\">" : "") . "
                                <span class=\"footer-logo-text\">University Of Lahore</span>
                            </div>
                        </td>
                    </tr>
                </table>
                ...
                <div class=\"footer-text\">
                    ...
                    Valid through academic year " . date('Y') . "-" . (date('Y')+1) . " | Reference: MES-MEM-{$userData['sap_id']}-" . date('Ymd') . "
                </div>
            </div>
        </body>
        </html>
