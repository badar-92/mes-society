...
class Functions {
    ...
    // Upload certificate (if you want a dedicated method, but we already implemented in admin)
// Optionally add a helper to get certificates by SAP ID
public function getCertificatesBySapId($sap_id) {
    try {
        $stmt = $this->db->prepare("SELECT c.*, e.title as event_title, comp.title as competition_title 
                                     FROM certificates c
        ...
        error_log("Get certificates by SAP ID error: " . $e->getMessage());
        return [];
    }
}
    ...
    public function getTeamMembersAll($department = '', $role = '', $year = '') {
    try {
        ...
        $sql .= " ORDER BY department, display_order, name";
        ...
        error_log("Get team members all error: " . $e->getMessage());
        return [];
    }
}
}
