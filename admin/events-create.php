...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($description) || empty($start_date) || empty($venue)) {
        $error = "Please fill all required fields (Title, Description, Start Date, and Venue)";
    } else {
        try {
            // Handle file upload
            $banner_image = null;
            ...
            if (!$error) {
                ...
                $checkTable = $db->query("SHOW TABLES LIKE 'events'");
                ...
                // Insert event
                $stmt = $db->prepare("
                    INSERT INTO events (title, description, event_type, category, start_date, end_date, venue, max_participants, registration_deadline, banner_image, status, created_by, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
                ");
...
// After successful event/competition creation, add this:
if ($result) {
    $newId = $db->lastInsertId();
    
    // DEBUG: Log what we're trying to do
    error_log("Attempting to create feed post for new item: ID=$newId, Title=$title, Status=$status");
    
  
    
    $success = "Event created successfully!" . ($status === 'published' ? " The event has been added to the feed." : "");
    $_POST = array();
} else {
                    $error = "Failed to create event. Database error occurred.";
                }
            }
                    ...
                    <form method="POST" action="" enctype="multipart/form-data">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" id="event_type" name="event_type" required>
                                        ...
                                        <option value="competition" <?php echo ($_POST['event_type'] ?? '') === 'competition' ? 'selected' : ''; ?>>Competition</option>
                                        <option value="social" <?php echo ($_POST['event_type'] ?? '') === 'social' ? 'selected' : ''; ?>>Social Event</option>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="category" name="category" 
                                           value="<?php echo htmlspecialchars($_POST['category'] ?? ''); ?>" placeholder="e.g., Technical, Cultural, Sports">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="status" name="status">
                                ...
                                <option value="published" <?php echo ($_POST['status'] ?? '') === 'published' ? 'selected' : ''; ?>>Published (Will appear in feed)</option>
                            </select>
                            <small class="text-muted">Select "Published" to automatically add this event to the society feed.</small>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Set registration deadline max to start date
    document.getElementById('start_date').addEventListener('change', function() {
