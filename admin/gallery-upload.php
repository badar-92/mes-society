...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['photos'])) {
    ...
    // Check current cover before upload
    $stmtCheckCover = $db->prepare("SELECT cover_photo FROM gallery_albums WHERE id = ?");
    $stmtCheckCover->execute([$albumId]);
    $hasCover = !empty($stmtCheckCover->fetchColumn());
    ...
    foreach ($_FILES['photos']['tmp_name'] as $key => $tmpName) {
        if ($_FILES['photos']['error'][$key] === UPLOAD_ERR_OK) {
            ...
            if (in_array($fileExtension, $allowedTypes) && $_FILES['photos']['size'][$key] <= 5 * 1024 * 1024) {
                if (move_uploaded_file($tmpName, $uploadFile)) {
                    try {
                        $caption = $_POST['captions'][$key] ?? '';
                        $stmt = $db->prepare("INSERT INTO gallery_photos (album_id, image_path, caption, uploaded_by, created_at) VALUES (?, ?, ?, ?, NOW())");
                        $stmt->execute([$albumId, $fileName, $caption, $_SESSION['user_id']]);
                        $uploadedCount++;
                        
                        // If album has no cover yet, set this as cover
                        if (!$hasCover && $uploadedCount == 1) {
                            $stmtCover = $db->prepare("UPDATE gallery_albums SET cover_photo = ? WHERE id = ?");
                            $stmtCover->execute([$fileName, $albumId]);
                            $hasCover = true; // avoid setting multiple times
                        }
                    } catch(PDOException $e) {
    ...
    <div class="row">
        ...
        <div class="col-md-9">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <div>
                    <a href="gallery-view.php?id=<?php echo $albumId; ?>" class="btn btn-secondary me-2"><i class="fas fa-eye me-2"></i>View Album</a>
                    <a href="gallery.php" class="btn btn-outline-secondary"><i class="fas fa-arrow-left me-2"></i>Back to Gallery</a>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-8">
                    <div class="card">
                        <div class="card-header"><h5 class="card-title mb-0"><i class="fas fa-upload me-2"></i>Upload Photos</h5></div>
                        <div class="card-body">
                            <form method="POST" action="" enctype="multipart/form-data" id="uploadForm">
                                <div class="mb-3">
                                    ...
                                    <small class="text-muted">You can select multiple images. Supported formats: JPG, PNG, GIF, WebP. Max file size: 5MB per image.</small>
                                </div>
                                ...
                                <div class="alert alert-info">
                                    ...
                                    <ul class="mb-0"><li>Select multiple photos using Ctrl+Click or Shift+Click</li><li>Add captions to identify your photos</li><li>Photos will be displayed in the order they are uploaded</li></ul>
                                </div>
                                <div class="text-end"><button type="submit" class="btn btn-accent btn-lg"><i class="fas fa-upload me-2"></i>Upload Photos</button></div>
                            </form>
                        </div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="card">
                        <div class="card-header"><h5 class="card-title mb-0"><i class="fas fa-info-circle me-2"></i>Album Info</h5></div>
                        <div class="card-body">
                            <h6><?php echo htmlspecialchars($album['title']); ?></h6>
                            <?php if($album['description']): ?><p class="text-muted"><?php echo htmlspecialchars($album['description']); ?></p><?php endif; ?>
                            <hr>
                            <div class="mb-2"><small class="text-muted">Status:</small> <span class="badge bg-<?php echo $album['status'] === 'active' ? 'success' : 'secondary'; ?>"><?php echo ucfirst($album['status']); ?></span></div>
                            <div class="mb-2"><small class="text-muted">Created:</small><br><small><?php echo date('F j, Y g:i A', strtotime($album['created_at'])); ?></small></div>
                            <?php
                            try {
                                $stmt = $db->prepare("SELECT COUNT(*) FROM gallery_photos WHERE album_id = ?");
                                $stmt->execute([$albumId]);
                                $photoCount = $stmt->fetchColumn();
                            } catch(PDOException $e) { $photoCount = 0; }
                            ?>
                            <div class="mb-2"><small class="text-muted">Current Photos:</small><br><small><strong><?php echo $photoCount; ?></strong> photos in this album</small></div>
                            <?php if($album['cover_photo']): ?>
                                <div class="mb-2"><small class="text-muted">Cover Photo:</small><br><small>Currently set</small></div>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    photosInput.addEventListener('change', function(e) {
        ...
        if (files.length > 0) {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                div.innerHTML = `<div class="d-flex justify-content-between align-items-start mb-2"><div><strong class="small">${fileName}</strong><br><small class="text-muted">${fileSize} MB</small></div><span class="badge bg-light text-dark">${i+1}</span></div><input type="text" class="form-control form-control-sm" name="captions[]" placeholder="Enter caption for this photo (optional)" maxlength="255">`;
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(e) {
        ...
        if (files.length === 0) { e.preventDefault(); alert('Please select at least one photo to upload.'); return false; }
        for (let i = 0; i < files.length; i++) {
            if (files[i].size > 5 * 1024 * 1024) { e.preventDefault(); alert('One or more files exceed the 5MB size limit. Please select smaller files.'); return false; }
        }
        return true;
    });
});
...
.btn-accent:hover { background-color: var(--accent-color-dark); border-color: var(--accent-color-dark); }
</style>
